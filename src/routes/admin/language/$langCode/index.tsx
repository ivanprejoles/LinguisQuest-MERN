...
export const Route = createFileRoute("/admin/language/$langCode/")({
  head: ({ params }) => ({ meta: [{ title: `Admin · ${params.langCode}` }] }),
  beforeLoad: (async ({ params }: { params: { langCode: string } }) => {
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {features.map((f) => {
            ...
            return f.enabled ? (
              <Link key={f.id} to="/admin/language/$langCode/foundation" params={{ langCode }}>
                {inner}
              </Link>
            ) : (
